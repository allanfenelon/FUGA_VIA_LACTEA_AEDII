...
// ALGORITIMO DE VERIFICAÇÃO DA EXISTENCIA DOS RECURSOS QUE EXPANDE A PASSAGEM
int verifica_ExistenciaRec(H *fila, int pos, char **recursos) {
    if (fila == NULL || pos < 0 || pos >= fila->tam) {
        return 0;  // Retorna 0 para indicar que não encontrou a combinação
    }

    N *aux = get_navedaFila(fila, pos);

    for (int j = 0; j < 3; j++) {
        int encontrado = 0;  // Inicialmente, assume que o recurso não foi encontrado
        ...
        // Se o recurso não foi encontrado, a combinação não está completa
        if (!encontrado) {
            // Libere a memória alocada para aux
